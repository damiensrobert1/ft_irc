...
void Server::handleRead(int fd)
{
	...
	while (true)
	{
		...
		if (n > 0)
		{
			std::string line = it->second.getInbuf().append(buf, (size_t)n);
			it->second.setInbuf(line);
...
void Server::processInputLines()
{
		...
		++it)
	{
		...
		while (true)
		{
			...
			std::string l = c.getInbuf().erase(0, pos + 1);
			c.setInbuf(l);
...
void Server::handleWrite(int fd)
{
	...
	while (!c.getOutbuf().empty())
	{
		...
		if (n > 0){
			std::string line = c.getOutbuf().erase(0, (size_t)n);
			c.setOutbuf(line);
