...
void Cmd::sendNumeric(int fd, const std::string& cmdOrNum,
	const std::string& msg) {
	std::string nick = "*";
	std::map<int, Client>::iterator it = clients.find(fd);
	if (it != clients.end() && !it->second.getNick().empty())
		nick = it->second.getNick();
	Utils::sendLine(fd, ":ircserv " + cmdOrNum + " " + nick + " " + msg,
		clients);
}
...
void Cmd::pass() {
	...
	std::string pass = parsed.getHasTrailing() ? parsed.getTrailing()
		: parsed.getArgs()[0];
	if (pass == password) {
		client.addAuthed();
...
static void broadcastNickChange(const std::string& line,
	const std::set<int>& targets, std::map<int, Client>& clients) {
	for (std::set<int>::const_iterator it = targets.begin();
		it != targets.end(); ++it) {
		Utils::sendLine(*it, line, clients);
	}
}

static void collectNickTargets(int clientFd,
	const std::map<std::string, Channel>& channels, std::set<int>& targets) {
	targets.insert(clientFd);
	for (std::map<std::string, Channel>::const_iterator it = channels.begin();
		it != channels.end(); ++it) {
		const Channel& ch = it->second;
		if (!ch.isMember(clientFd))
			continue;
		const std::set<int>& memberSet = ch.getMembers();
		for (std::set<int>::const_iterator mit = memberSet.begin();
			mit != memberSet.end(); ++mit) {
			targets.insert(*mit);
		}
	}
}
...
void Cmd::nick() {
	...
	std::string newNick = parsed.getHasTrailing() ? parsed.getTrailing()
		: parsed.getArgs()[0];
	if (newNick.empty()) {
	...
	if (oldNick.empty())
		return;
	std::string line = ":" + oldNick + "!" + client.getUser() + "@"
		+ client.getIp() + " NICK :" + client.getNick();
	std::set<int> targets;
	collectNickTargets(client.getFd(), channels, targets);
	broadcastNickChange(line, targets, clients);
}
...
void Cmd::userhost() {
	...
	for (size_t i = 0; i < limit; ++i) {
		...
		if (!t)
			continue;
		std::string item = t->getNick() + "=+"
			+ (t->getUser().empty() ? "unknown" : t->getUser()) + "@"
			+ (t->getIp().empty() ? "0.0.0.0" : t->getIp());
		if (!reply.empty())
			reply += " ";
		reply += item;
	}
...
void Cmd::broadcastToChannel(const Channel& ch, int except_fd,
...
void Cmd::sendNamesList(Client& c, Channel& ch) {
	...
	for (std::set<int>::const_iterator it = memberSet.begin();
		...
		if (ch.isOp(mfd))
			names += "@";
		names += iter->second.getNick().empty() ? "*"
			: iter->second.getNick();
	}
...
bool Cmd::checkJoinPermissions(Client& client, Channel& ch,
	const std::string& providedKey) {
	if (ch.getHasLimit() && ch.getMemberCount() >=
		static_cast<size_t>(ch.getUserLimit()) &&
		!ch.isMember(client.getFd())) {
		this->sendNumeric(client.getFd(), "471",
			ch.getName() + " :Cannot join channel (+l)");
		return false;
	}
	if (ch.getInviteOnly() && !ch.isMember(client.getFd())) {
		if (!ch.isInvited(client.getFd())) {
			this->sendNumeric(client.getFd(), "473",
				ch.getName() + " :Cannot join channel (+i)");
			return false;
		}
	}
	if (ch.getHasKey() && !ch.isMember(client.getFd())) {
		if (providedKey != ch.getKey()) {
			this->sendNumeric(client.getFd(), "475",
				ch.getName() + " :Cannot join channel (+k)");
			return false;
		}
	}
	return true;
...
void Cmd::completeJoin(Client& client, Channel& ch) {
	std::string joinLine = ":" + client.getPrefix() + " JOIN :"
		+ ch.getName();
	Utils::sendLine(client.getFd(), joinLine, this->clients);
	this->broadcastToChannel(ch, client.getFd(), joinLine);
	if (ch.getTopic().empty())
		this->sendNumeric(client.getFd(), "331",
			ch.getName() + " :No topic is set");
	else
		this->sendNumeric(client.getFd(), "332",
			ch.getName() + " :" + ch.getTopic());
	this->sendNamesList(client, ch);
}
...
void Cmd::join() {
	...
	std::string chanNameStr = parsed.getHasTrailing() ? parsed.getTrailing()
		: parsed.getArgs()[0];
	if (chanNameStr.empty() || chanNameStr[0] != '#') {
		sendNumeric(client.getFd(), "479",
			chanNameStr + " :Illegal channel name");
		return;
	}
	...
	for (size_t i = 0; i < channelNames.size(); ++i) {
		...
		if (chanName.empty() || chanName[0] != '#') {
			sendNumeric(client.getFd(), "479",
				chanName + " :Illegal channel name");
			return;
		}
		Channel& ch = getOrCreateChannel(chanName);
		if (!checkJoinPermissions(client, ch, providedKey))
			continue;
...
void Cmd::part() {
	...
	std::string reason = parsed.getHasTrailing() ? parsed.getTrailing()
		: "Leaving";
	for (size_t i = 0; i < channelNames.size(); ++i) {
		std::string chanName = channelNames[i];
		...
		if (!ch.isMember(client.getFd())) {
			sendNumeric(client.getFd(), "442",
				chanName + " :You're not on that channel");
			continue;
		}
		std::string line = ":" + client.getPrefix() + " PART " + chanName
			+ " :" + reason;
		const std::set<int>& memberSet = ch.getMembers();
		for (std::set<int>::const_iterator mit = memberSet.begin();
			mit != memberSet.end(); ++mit) {
...
void Cmd::whois() {
	...
	sendNumeric(client.getFd(), "311", t->getNick() + " " + t->getUser()
		+ " " + t->getIp() + " * :" + t->getRealname());
...
void Cmd::privmsg() {
	...
	if (!target.empty() && target[0] == '#') {
		...
		std::string line = ":" + client.getPrefix() + " PRIVMSG " + target
			+ " :" + text;
		...
		Utils::sendFromClient(dst->getFd(), client, "PRIVMSG",
			dst->getNick() + " :" + text, clients);
	}
}
...
void Cmd::kick() {
	...
	if (!ch.isOp(client.getFd())) {
		sendNumeric(client.getFd(), "482",
			chanName + " :You're not channel operator");
		return;
	}
	Client* victim = Utils::findByNick(nick, clients);
	if (!victim || !ch.isMember(victim->getFd())) {
		sendNumeric(client.getFd(), "441",
			nick + " " + chanName + " :They aren't on that channel");
		return;
	}
	std::string kickLine = ":" + client.getPrefix() + " KICK " + chanName
		+ " " + victim->getNick() + " :" + reason;
...
void Cmd::invite() {
	...
	if (ch.isMember(dst->getFd())) {
		sendNumeric(client.getFd(), "443",
			nick + " " + chanName + " :is already on channel");
		return;
	}
	ch.addInvited(dst->getFd());
	sendNumeric(client.getFd(), "341", nick + " " + chanName);
	Utils::sendFromClient(dst->getFd(), client, "INVITE",
		dst->getNick() + " :" + chanName, clients);
}
...
void Cmd::topic() {
	...
	if (!parsed.getHasTrailing()) {
		if (ch.getTopic().empty())
			...
			sendNumeric(client.getFd(), "332",
				chanName + " :" + ch.getTopic());
		return;
	}
	...
	std::string line = ":" + client.getPrefix() + " TOPIC " + chanName + " :"
		+ ch.getTopic();
...
void Cmd::mode() {
	...
	std::string target = parsed.getArgs()[0];
	if (target.empty() || target[0] != '#') {
		sendNumeric(client.getFd(), "472",
			":Only channel MODE is supported here");
		return;
	}
	std::map<std::string, Channel>::iterator it = channels.find(target);
	if (it == channels.end()) {
	...
	if (parsed.getArgs().size() == 1) {
		...
		if (ch.getHasKey())
			params += " " + ch.getKey();
		if (ch.getHasLimit())
			params += " " + Utils::toString(ch.getUserLimit());
		sendNumeric(client.getFd(), "324",
			ch.getName() + " " + modes + params);
		return;
	}
	if (!ch.isMember(client.getFd())) {
		sendNumeric(client.getFd(), "442",
	...
	for (size_t i = 0; i < modeStr.size(); ++i) {
		...
		if (m == 'i') {
			...
			if (adding) {
				...
				ch.setUserLimit(std::max(0,
					::atoi(parsed.getArgs()[argi++].c_str())));
				applied += "+l";
			...
			if (!dst || !ch.isMember(dst->getFd())) {
				sendNumeric(client.getFd(), "441", nick + " " + ch.getName()
...
void Cmd::tryRegister() {
	...
	sendNumeric(client.getFd(), "001",
		":Welcome to ft_irc " + client.getPrefix());
	sendNumeric(client.getFd(), "002", ":Your host is ircserv");
	sendNumeric(client.getFd(), "003",
		":This server is a minimal ft_irc implementation");
}